...
plotDbCharacteristics <- function(rootOutputFolderName = "MethodsLibraryPleEvaluation_", rootFolder = "s:/") {
    ...
    populationCount$label <- sprintf("%.2fM persons", populationCount$subjectCount / 1e6)
    ...
    noStrip <- ggplot2::theme(strip.text = ggplot2::element_blank(),
                              legend.position = "none")
    plot <- gridExtra::grid.arrange(grid::textGrob(""), legend3, legend4, grid::textGrob(""), grid::textGrob(""),
                                    plot2 + noStrip, plot3  + noStrip, plot4  + noStrip, plot5  + noStrip, plot6,
                                    ncol = 5, nrow = 2, widths = c(200, 100, 100, 200, 200),
                                    heights = c(100, 400),
                                    respect = FALSE)
    ggplot2::ggsave(filename = file.path(exportFolders[1], "DbOverview.png"), plot = plot, with = 10, height = 7, dpi = 400)
}
